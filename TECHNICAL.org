...
* Reproducibility helpers
  ...
  The =daggerlib-tangle-test= helper returns the tangle path for
  library test command files.  Each =dagger call= bash block in the
  =src/*.org= files tangles to =tests/commands/<name>=.  =run.sh=
  executes the bash blocks and populates =#+RESULTS:=.  Tangling
  then writes those results to =tests/expected/= via noweb
  references.  The test runner iterates over command files, runs
  each one, and compares stdout to the expected output.

  Example org files (=examples/*/readme.org=) tangle their tests
  into the example's own =tests/commands/= and =tests/expected/=
  directories using relative paths with =:mkdirp yes=.  The test
  runner discovers these and runs them with =cwd= set to the
  example directory so =dagger call= loads the example module.
