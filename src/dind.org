...
#+begin_src python :tangle lib/dind.py :noweb yes :exports none
...
* Running the test suite inside DinD
  ...
  then execute =./test-host.sh= with dockerd available.  Some tests
  drive the org-babel run pipeline (=run-host.sh=) directly to check the
  tooling itself, so the container also carries emacs, git, and the
  =.tangle-deps= cache — letting those tests run in-place instead of
  paying a nested =dagger call= to reach an emacs.
...
** Running org-babel blocks
   ...
   =dagger call=) and returns only the modified files.  An example
   block calls its own module, which declares =lib= as a local
   dependency at =../..=.  The example's =dagger call= reaches =lib=
   only when an empty =.git= is mounted alongside the source, so the
   runner mounts one (as =dind_run_tests= does).
   ...
   Running a committed example through the runner must reproduce its
   results.  Re-running re-saves the file even when its
   contents are unchanged, so it always turns up in the returned diff and
   a bare diff would mislead; instead we read one block's result,
   =flask_gunicorn_check=, back from the run and pin that it carries its
   value rather than an empty line.  We assert the example is present
   first, so a removed fixture fails loudly.

   #+NAME: dind_run_org_keeps_example_results
   ...
   #+RESULTS[1b0e2a2193d6ee723235797db1163c23339c5bc8]: dind_run_org_keeps_example_results
   : flask_gunicorn_check=[gunicorn config ok]
...
** Exporting org files to HTML
  ...
  #+RESULTS[75f542a33f30c0c7b0fb195494cc3b66b8ed2017]: dind_docker_info
  : linux
