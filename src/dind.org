...
as the DNS server for containers.  After =dockerd= creates the
bridge, add iptables DNAT rules that forward DNS queries arriving at
=172.17.0.1:53= to the real DNS server.  Containers managed by the
inner Docker — including a nested Dagger engine — can always reach the
bridge gateway, so DNS works transparently.
...
* Docker daemon sidecar
  ...
  forwarding and retry-on-corruption logic on top.
