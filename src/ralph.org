...
* Standalone log filter
  ...
  The log filter is also available as a standalone script for use in
  containers where =clk= is not installed ([[file:dev.org][mode 2/3]] testing). It
  contains the same logic as =clk ralph log-filter= but only uses
  stdlib modules.
