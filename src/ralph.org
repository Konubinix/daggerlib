...
for raw in sys.stdin:
    ...
    if m:
        out("\U0001f916", m.group(1))
        continue
    # event emissions
    if "Event emitted" in raw:
        m2 = re.search(r"Event emitted: ([\w.]+)", raw)
        out("\U0001f4e8", f"event: {m2.group(1)}" if m2 else "event emitted")
        continue
    # git commits (before generic tool match)
    if "git commit" in raw:
        m = re.search(r"git commit -m .(.{3,120}?)(?:\\n|\\|\"\"|\x27\x27|$)", raw)
        if m:
            out("\U0001f4be", f"commit: {m.group(1).strip().strip(chr(34))}")
        else:
            out("\U0001f4be", "commit")
        continue
    # tool calls
    ...
    if m:
        ...
        if mc:
            ...
            out("\U0001f4bb", f"{tool}: {cmd}")
        elif mf:
            out("\U0000270f\U0000fe0f", f"{tool}: {mf.group(1)}")
        elif mp:
            out("\U0001f50d", f"{tool}: {mp.group(1)}")
        continue
    # test results
    m = re.search(r"(\d+ passed[^\"\\\\]*)", raw)
    if m:
        out("\u2705" if "failed" not in m.group(1) else "\u274c", m.group(1))
        continue
