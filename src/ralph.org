...
* Running ralph in a container
  ...
  The =ralph= function orchestrates five stages, each handled by a
  private helper:
  ...
  already installed (test frameworks, build tools, etc.). The =ralph=
  function adds the ralph layer on top via these stages.

  Exit code 2 from ralph means "max iterations reached", which is an
  expected outcome — not an error.

  The wrapper script starts ralph in the background and optionally
  polls Consul for stop signals and log-level changes. It captures
  the exit code for later inspection.

  #+begin_src sh :tangle lib/ralph-wrapper.sh :tangle-mode (identity #o755)
  #!/bin/sh
  cd "$WORK_DIR"
...
func: debian
args:
  distro-packages: [python3-yaml]
chain:
  - with-file:
      path: /tmp/ralph.yml
      source: "${ROOT}/src/ralph.yml"
  - with-exec: [python3, -c, "import yaml; d=yaml.safe_load(open('/tmp/ralph.yml')); print(d['event_loop']['starting_event']); print(len(d['hats']))"]
  - stdout
  #+end_src

  #+RESULTS[262bcf831e0a967619ecc4e40c62ffb619ff1319]: ralph_yml_valid
  :results:
  #+begin_src bash :exports code :eval no-export
  dagger call debian --distro-packages=python3-yaml with-file --path=/tmp/ralph.yml --source="${ROOT}/src/ralph.yml" with-exec --args="python3","-c","import yaml; d=yaml.safe_load(open('/tmp/ralph.yml')); print(d['event_loop']['starting_event']); print(len(d['hats']))" stdout
  #+end_src

  #+RESULTS[75a8ecbfcf258bc28782db45878319966b98b992]:
  : work.start
  : 2
  :end:

  The log filter is tested by piping a fixture of real ralph output
  through =ralph-log-filter= and comparing the result.

  #+NAME: log_filter_output
  #+begin_src fixtureyaml
shell: ralph-log-filter < "${ROOT}/tests/ralph_log_sample.txt" 2>/dev/null | sed 's/\x1b\[[0-9;]*m//g'
  #+end_src

  #+RESULTS[b7b743a99cb005c92dd5c68ad6611b266c4c7d69]: log_filter_output
  :results:
  #+begin_src bash :exports code :eval no-export
  ralph-log-filter < "${ROOT}/tests/ralph_log_sample.txt" 2>/dev/null | sed 's/\x1b\[[0-9;]*m//g'
  #+end_src

  #+RESULTS[2c3159386493baf6f6912e7c487782d8b35d7ace]:
...
* Log filter
  ...
  The log filter is a standalone script that colorizes and summarizes
  ralph/dagger output from stdin. It only uses stdlib modules so it
  works in containers where =clk= is not installed.

  #+begin_src python :tangle ../tests/ralph-log-filter :tangle-mode (identity #o755) :comments no :padline no
  #!/usr/bin/env python3
