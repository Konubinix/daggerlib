...
def out(icon, msg):
    ...
    line = f"{c}{icon}{RST} {msg}"
    # truncate on visible width (exclude ANSI escape bytes)
    visible = f"{icon} {msg}"
    if len(visible) > W:
        excess = len(visible) - W
        line = line[:-excess]
